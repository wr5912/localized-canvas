...
export default function ModelSelector({
  ...
  const allAllowedModels = ALL_MODELS.filter((model) => {
    ...
    if (
        ...
        process.env.NEXT_PUBLIC_DEEPSEEK_ENABLED === "false"
    ) {
      return false;
    }

    if (
        model.name.includes("vllm-") &&
        process.env.NEXT_PUBLIC_VLLM_ENABLED === "false"
    ) {
      return false;
    }
  ...
  const deepseekModelGroup = allAllowedModels.filter(
      (m) => m.config.provider === "deepseek"
  );
  const vllmModelGroup = allAllowedModels.filter(
      (m) => m.config.provider === "vllm"
  );
  return (
      ...
      <PopoverContent className="min-w-[180px] w-[280px] p-0 shadow-md rounded-md">
        <Command>
          <CommandList>
            ...
            {deepseekModelGroup.length > 0 && (
                <CommandGroup heading="DeepSeek" className="w-full">
                  {deepseekModelGroup.map((model) => {
                    const config = modelConfigs[model.name];
                    return (
                        <CommandModelItem
                            key={model.name}
            ...
            {vllmModelGroup.length > 0 && (
                <CommandGroup heading="VLLM Local" className="w-full">
                  {vllmModelGroup.map((model) => {
                    const config = modelConfigs[model.name];
                    return (
                        <CommandModelItem
                            key={model.name}
