...
const DEEPSEEK_MODELS: ModelConfigurationParams[] = [
  {
    name: "deepseek-chat",
    label: "deepseek-chat",
    config: {
...
const VLLM_MODELS: ModelConfigurationParams[] = [
  {
    name: "vllm-local",
    label: "VLLM Local Model",
    config: {
      provider: "openai",  // VLLM 也兼容 OpenAI API
      temperatureRange: {
        min: 0,
        max: 1,
        default: 0.5,
...
type DEEPSEEK_MODEL_NAMES = (typeof DEEPSEEK_MODELS)[number]["name"];
type VLLM_MODEL_NAMES = (typeof VLLM_MODELS)[number]["name"];
export type ALL_MODEL_NAMES =
  ...
  | OLLAMA_MODEL_NAMES
  | GROQ_MODEL_NAMES
  | DEEPSEEK_MODEL_NAMES  // 添加这行
  | VLLM_MODEL_NAMES;     // 添加这行

export const DEFAULT_MODEL_NAME: ALL_MODEL_NAMES = DEEPSEEK_MODELS[1].name;
export const DEFAULT_MODEL_CONFIG: CustomModelConfig = {
  ...DEEPSEEK_MODELS[1].config,
  temperatureRange: { ...DEEPSEEK_MODELS[1].config.temperatureRange },
  maxTokens: { ...DEEPSEEK_MODELS[1].config.maxTokens },
};
